...
class FeedScreen extends React.PureComponent {
  ...
  fetchCovers = async () => {
    ...
    if (result.success) {
      const hasNextPage = !isEmpty(
        result && result.pagination && result.pagination.links.next
      );
  ...
  fetchNextPage = async () => {
    ...
    if (result.success) {
      const hasNextPage = !isEmpty(
        result && result.pagination && result.pagination.next
      );
        ...
        <FlatList
          ...
          renderItem={({ item }) => (
            <ListItem
              ...
              onPress={() =>
                navigation.navigate('CoverDetails', { cover: item })
              }
            />
          )}
